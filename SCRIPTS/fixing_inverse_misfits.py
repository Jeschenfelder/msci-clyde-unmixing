...
parser.add_argument('path', type=str, help='Name of file of faulty inverse')
args = parser.parse_args()
filename = args.path

field_to_add = np.loadtxt(filename)
first_split = filename.split('/')
split_string = first_split[-1].split('_')
worked_exp_lambda = float(split_string[1])
elem = str(split_string[0])
nx = 63
ny = 55
...
################## running roughness and misfit calculations #####################################
blocks[active_blocks] = field_to_add
x_rough, y_rough  = cost_roughness(blocks, active_blocks,block_width,block_height) #calculating roughness of output
final_misfit = data_misfit(field_to_add.reshape(flat_shape), elem) #calculating misfit of output
tradeoff_array = np.array([worked_exp_lambda, x_rough, y_rough,final_misfit])
tradeoff_path = '../DATA/INVERSE_RESULTS/' + str(elem) + '_' + str(worked_exp_lambda) + '_roughness_misfit.txt'
