...
Synthetic input is a binary chequerboard of width specified by user

The set up of the inversion (e.g. topographic, drainage) is the same as used for
the true inversion.

e.g.: python invert_synthetic.py [nx] [ny] [n_lam] [cheq_size]
...
def chequerboard(w,h,val1=1e4,val2=1e3):
    """Returns a chequerboard pattern"""
...
chequ_size=float(sys.argv[4])
# Generate downstream data and priors
source_region = chequerboard(chequ_size,chequ_size)
...
out_prefix='synth_outputs/smoothed_'+str(nx)+'x'+str(ny)+'_chequer_'+str(chequ_size)+'_'
...
for j in np.arange(n_lam):
    ...
    if (counter == array_index): # <<<<< Uncomment if running as a batch job
        ...
        # plt.imshow(expanded,origin='lower',vmin=1e3,vmax=1e4,norm=LogNorm())
        # plt.colorbar()
        # plt.title("Output")
        # plt.show()
        ...
        # plt.imshow(source_region,origin='lower',vmin=1e3,vmax=1e4,norm=LogNorm())
        # plt.colorbar()
        # plt.title("Input synthetic data")
        # plt.show()
