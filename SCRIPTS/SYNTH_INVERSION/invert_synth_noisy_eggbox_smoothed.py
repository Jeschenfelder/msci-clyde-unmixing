...
synth_clean = downstream_synth_data(source_region) # no noise
sd=np.std(np.log10(synth_clean))
noise = np.random.normal(0,sd*0.223,synth_clean.shape) # sd*0.223 = 5% variance as noise
synth_obs = synth_clean*(10**noise)
...
out_prefix='synth_outputs/noisy_smoothed_'+str(nx)+'x'+str(ny)+'_eggbox_'+str(eggbox_size)+'_'
